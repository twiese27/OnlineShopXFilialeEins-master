...
with oracledb.connect(user=user, password=pw, dsn=dsn) as connection:
    with connection.cursor() as cursor:
        ...
        JOIN EMPLOYEE_HAS_ADDRESS
        ON EMPLOYEE.EMPLOYEE_ID = EMPLOYEE_HAS_ADDRESS.EMPLOYEE_ID
        JOIN ADDRESS
        ON EMPLOYEE_HAS_ADDRESS.ADDRESS_ID = ADDRESS.ADDRESS_ID
        """

        export_join_to_csv(join_query, "employee_to_address_join.csv", cursor)
